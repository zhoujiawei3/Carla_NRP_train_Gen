...
parser.add_argument('--color', type=str, default='0,255,255', help='color name')
parser.add_argument('--output_dir', type=str, default='C:/outputs/DTN/', help='output directory path')
parser.add_argument('--vehicle', type=str, default='vehicle.audi.etron', help='carla vehicle blueprint id, e.g. vehicle.audi.etron')
...
def sensor_callback(world,camera,ego_vehicle,spectator,sensor_data, sensor_queue, sensor_name):
    ...
    if 'camera' in sensor_name:
        ...
        sensor_data.save_to_disk(os.path.join(output_dir, new_img_filename))
        generate_mask(os.path.join(output_dir, new_img_filename))
        # im = cv2.imread(os.path.join(os.path.join(f'E:/outputs/DTN/differentColor/{color_name}', new_img_filename)))
        # im_resized = cv2.resize(im, (800, 800))
        # new_filename = "data" + str(count_img + 1) + ".npz"
        # dest_path = os.path.join('E:/outputs/DTN/_train', new_filename)
